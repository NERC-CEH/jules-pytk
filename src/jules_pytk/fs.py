...
class AsciiFile(File):
    ...
    def data(self, new: list[float] | numpy.ndarray) -> None:
        ...
        if self.data is None and new.shape != self.data.shape:
            logger.warning(f"Data has changed shape! (new) {new.shape} =/= {self.data.shape} (old)")
...
def JulesIOFile(file_ext: str) -> type[AsciiFile] | type[NetcdfFile] | type[NamelistFile]:
...
class NamelistsDirectory(Directory):
    ...
    def _read(cls, path: Path) -> Self:
        ...
        namelist_files = {
            namelist: NamelistFile.read((path / namelist).with_suffix(".nml")) for namelist in namelists
        }
    ...
    def _detach(self) -> Self:
        ...
        detached_files = {
            getattr(self, namelist).detach() for namelist in namelists
        }
    ...
    def to_dict(self) -> dict:
        ...
        return {
            namelist: getattr(self, namelist).data.todict()
            for namelist in namelists
        }
